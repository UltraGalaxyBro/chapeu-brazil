...
import { BreadcrumbItem, ProductEdit } from '@/types';
...
type FormType = {
    ...
    qualities: number[];
    images: File[];
    existing_images: { id: number; url: string; order: number }[];
    image_order: number[];
    variants: {
        id?: number;
        color_id: number;
...
export default function EditProduct({ product, productImages, categories, brands, colors, sizes, qualities }: ProductEdit): React.ReactElement {
    ...
    const { data, setData, post, processing, errors } = useForm<FormType>({
        ...
        qualities: product.qualities.map(q => q.id),
        existing_images: productImages
    ...
    useEffect(() => {
        const sortedImages = [...data.existing_images].sort((a, b) => a.order - b.order);
        setPreviewImages(sortedImages.map(img => img.url));
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            Array.from(e.target.files).forEach((file) => {
                ...
                newImageOrder.push(newImages.length + data.existing_images.length - 1);
            });
    ...
    const removeImage = (index: number) => {
        ...
        if (index < newExistingImages.length) {
            ...
            newImages.splice(index - newExistingImages.length, 1);
        }
        ...
        setData({ ...data, images: newImages, existing_images: newExistingImages, image_order: adjustedOrder });
        setPreviewImages(newPreviews);
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        const newExistingImages = [...data.existing_images];
        const [movedItem] = newPreviewImages.splice(result.source.index, 1);
        newPreviewImages.splice(result.destination.index, 0, movedItem);
        
        for (let i = 0; i < newExistingImages.length; i++) {
            const imgUrl = newExistingImages[i].url;
            const newIndex = newPreviewImages.indexOf(imgUrl);
            if (newIndex >= 0) {
                newExistingImages[i] = { ...newExistingImages[i], order: newIndex };
            }
        }
        
        const newOrders = newPreviewImages.map((_, index) => index);
        
        setData({
            ...data,
            existing_images: newExistingImages,
            image_order: newOrders
        });
    ...
    const updateVariant = (index: number, field: string, value: number | string) => {
        ...
        if ((field === 'color_id' || field === 'size_id') &&
            checkForDuplicateVariant(data.variants,
                updatedVariant.color_id,
                updatedVariant.size_id,
                index)) {
    ...
    const validateStep = (step: number): boolean => {
        ...
        switch (step) {
            case 1:
                ...
                if (data.price <= 0) {
                    toast.error('O preço deve be maior que zero.');
                    return false;
                }
            ...
            case 2:
                ...
                return true;
            case 3:
                if (data.existing_images.length === 0 && data.images.length === 0) {
            ...
            case 4:
                ...
                toast.success('Variantes validadas! Atualizando produto...');
                return true;
            default:
                return true;
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (currentStep === totalSteps && validateStep(currentStep)) {
            ...
            Object.entries(data).forEach(([key, value]) => {
                if (key === 'images' || key === 'existing_images') return;
            ...
            formData.append('image_order', JSON.stringify(
                data.existing_images.map(img => img.order).concat(
                    data.images.map((_, i) => data.existing_images.length + i)
                )
            ));
    ...
    useEffect(() => {
        if ((data.images.length + data.existing_images.length) > 0 && data.image_order.length === 0) {
            const sortedExistingImages = [...data.existing_images].sort((a, b) => a.order - b.order);
            const initialOrder = sortedExistingImages.map(img => img.order);
    
            if (data.images.length > 0) {
                const maxOrder = Math.max(...initialOrder, 0);
                for (let i = 0; i < data.images.length; i++) {
                    initialOrder.push(maxOrder + i + 1);
                }
            }
    
            setData('image_order', initialOrder);
        }
    }, [data.images, data.existing_images, data.image_order.length, setData]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Editando ${product.name}`} />
                        ...
                        <div className="flex flex-col sm:flex-row justify-between mt-8 gap-4">
                            ...
                            {currentStep < totalSteps ? (
                                ...
                                >
                                    ...
                                    {processing ? 'Atualizando...' : data.variants.length > 0 ? 'Atualizar produto' : 'Adicione uma variante'}
                                </Button>
                            )}
                        </div>
                    </form>
